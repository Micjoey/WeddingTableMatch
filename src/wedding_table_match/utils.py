...
def write_sample_guests_csv(path: str | Path):
    """Write a sample guests CSV including all supported fields, including partner."""
    header = [
...
def write_sample_relationships_csv(path: str | Path):
    ...
    header = ["a","b","relation","strength","notes"]
    rows = [
...
# Options for sidebar UI to control seating assignment logic
SEATING_OPTIONS = {
    "maximize_known": {
